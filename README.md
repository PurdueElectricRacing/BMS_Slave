...
- Gather voltage and temperature data from each cell (or a portion for temps) in a module
...
The BMS slave's `STM32` uses `FreeRTOS` to manage scheduling and execution of code. By doing this, the application can run multiple threads "simultaneously" allowing for easier development. Upon startup, a few tasks are spawned. In order, the functions started are:
...
2. The slave is then woken at some point via an interrupt and enters the init state.
...
The code for this portion is quite simple. State machines are easy to follow by nature as they progress in a (typically) logical pattern. The code for the main task can be found on line 211 of `bms.c` at the time of writing.
...
1. The task initializes communication with the `BMIC` by pulsing the enable line.
2. The slave module writes cell over/undervoltage values using the `WRCFGA` command.
3. The `BMIC` enters a conversion mode in which cells 1 to 7 are polled for conversion.
...
5. The `BMIC` enters a conversion mode in which *all* cells are polled for conversion.

6. The slave module requests the voltage register groups and saves the data for processing.
...
1. The task declares a CAN message struct to allow for the creation of a header message.
...
6. The message is officially sent using HAL, and the task runs its delay.
...
1. The task initializes 2 [`LTC2497`](https://www.mouser.com/datasheet/2/609/2497fb-1267645.pdf) ICs by attempting to communicate over I2C.
